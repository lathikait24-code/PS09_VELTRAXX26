...
## Files
- `download_data.py` — downloads CIFAR-10 (and MNIST) via torchvision/TensorFlow
- `quantize_model.py` — loads the trained `cifar_model.keras`, evaluates FP32 accuracy, converts to `model_fp32.tflite`, applies INT8 post-training quantization (calibrated on 200 training samples) to produce `model_int8.tflite`, evaluates INT8 accuracy, and writes `results.txt`

## Not yet included
- CNN training script (produces `cifar_model.keras` — required input to `quantize_model.py`)
- TFLiteimport tensorflow as tf
...
with open('results.txt', 'w') as f:
    ...
    f.write(f"INT8 Size: {int8_size} bytes\n")[download_data.py](https://github.com/user-attachments/files/31569060/download_data.py)

    f.write(f"Compression: {fp32_size/int8_size:.2f}x\n")
print("Results saved to results.txt") Micro conversion script (C array export for deployment)
  [quantize_model.import torchvision
...
print("Done!")py](https://github.com/user-attachments/files/31569015/quantize_model.py)
